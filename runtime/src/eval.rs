...
use super::{
  ...
  vec_kind, Escape, Function, FunctionKind, List, Map, Object, Reader, Scope, SpecialForm, Symbol,
  Value, Vec,
};
      ...
      Some(state) => match state {
        EvalState::Eval => {
          ...
          if value.kind() == &symbol_kind(stack.scope.front().unwrap().clone()) {
            ...
            if let Some(value) = stack.scope.front().unwrap().get(string.value().inner()) {
            ...
            if let Some(value) = vec.pop() {
              ...
              stack
                .value
                .push_front(new_vec(stack.scope.front().unwrap().clone()).into_value());
            ...
            if let Some((key, value)) = key_values.0.pop_back() {
              ...
              stack
                .value
                .push_front(new_map(stack.scope.front().unwrap().clone()).into_value());
        ...
        EvalState::Call => {
          ...
          if callable.kind() == &function_kind(stack.scope.front().unwrap().clone()) {
            ...
            stack.value.push_front(callable);
            stack.value.push_front(args.into_value());
          } else if callable.kind() == &special_form_kind(stack.scope.front().unwrap().clone()) {
