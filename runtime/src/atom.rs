...
use super::{
  add_external_function, new_kind, new_object, new_persistent_vector_from, nil_value,
  scope_get_with_kind, scope_set, Kind, Map, Object, PersistentScope, PersistentVector, Value,
};
...
impl Atom {
  ...
  fn inner_mut(&mut self) -> &mut Gc<dyn Value> {
    unsafe { &mut **self.atomic_ptr.get_mut() }
  }
  ...
  pub(crate) fn init_scope(scope: &Gc<Object<PersistentScope>>) -> Gc<Object<PersistentScope>> {
    let mut new_scope = add_external_function(scope, "atom.new", vec!["value"], atom_new);
    new_scope = add_external_function(&new_scope, "atom.get", vec!["atom"], atom_get);
    new_scope = add_external_function(&new_scope, "atom.set", vec!["atom", "value"], atom_set);
    new_scope
  }
}

#[inline]
pub fn atom_kind(scope: &Gc<Object<PersistentScope>>) -> &Gc<Object<Kind>> {
  scope_get_with_kind::<Kind>(scope, "Atom").expect("failed to get Atom Kind")
}

#[inline]
pub fn new_atom(scope: &Gc<Object<PersistentScope>>, value: Gc<dyn Value>) -> Gc<Object<Atom>> {
...
pub fn atom_set(
  ...
  let atom = args
    ...
    .downcast_ref::<Object<Atom>>()
    .expect("Failed to downcast to Atom");
  atom.update(atom_value_from_args(
    scope,
    &new_persistent_vector_from(scope, args.pop_front()),
  ));
