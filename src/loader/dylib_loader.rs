...
use gc::Gc;
use runtime::{
  new_map, new_string, nil_value, Map, Object, PersistentScope, PersistentVector, Value,
};

use super::super::{new_dylib, new_module};

#[inline]
pub fn dylib_loader_lisp_fn(
  ...
  if filename.ends_with(".so") {
    ...
    if cache.has(&path_value) {
      ...
      exports.set(
        ...
        new_dylib(scope, path.to_str().unwrap()).into_value(),
      );
