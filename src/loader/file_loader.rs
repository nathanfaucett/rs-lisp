...
pub fn file_loader(
  ...
  if filename.starts_with(".") || filename.starts_with("/") || filename.starts_with("\\") {
    ...
    if cache.has(&path_value) {
      ...
      module_scope = scope_set(&module_scope, "module", module.clone().into_value());
      module_scope = scope_set(&module_scope, "__filename", path_value.clone());
      module_scope = scope_set(
        &module_scope,
        "__dirname",
        new_string(
          scope,
      ...
      module_scope = add_external_macro(
        &module_scope,
        "import",
        vec!["...imports", "module_path"],
        import,
      );
      module_scope = add_external_macro(&module_scope, "export", vec!["...exports"], export);
